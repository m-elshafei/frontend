...
<div class="max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-6 text-right">
        <h3 class="text-3xl font-bold text-gray-700">إضافة مورد جديد</h3>
        ...
        <form action="{{ route('suppliers.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline text-right" id="email" name="email" type="email" value="{{ old('email') }}">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="phone">الهاتف</label>
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline text-right" id="phone" name="phone" type="text" value="{{ old('phone') }}">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="balance">الرصيد الافتتاحي</label>
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline text-right" id="balance" name="balance" type="number" step="0.01" value="{{ old('balance', 0) }}">
                </div>
                <div class="mb-4 md:col-span-2">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="address">العنوان</label>
                    <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline text-right" id="address" name="address" rows="3">{{ old('address') }}</textarea>
                </div>
            </div>
